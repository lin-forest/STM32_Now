...
static TB6612_Motor_t tb6612_motor; // 将电机对象设为任务本地静态变量

void TB6612_DC_Task(void *argument)
{
    Motor_PID_Init(); // 初始化PID参数

    CommandMsg_t cmdMsg; // 用于接收命令的消息结构体
    ...
    TB6612_Motor_Init(&tb6612_motor, MOTOR1_TIM_HANDLE, MOTOR1_TIM_CHANNEL,\
                ...
                MOTOR1_DEAD_ZONE, TB6612_MOTOR_STOP_BRAKE);
    ...
    for(;;)
    {
        ...
        // 1. 非阻塞地从统一的指令队列(MotorQueueHandle)中获取指令
        ...
        // 3. 无论有无新指令，都以固定频率执行PID闭环控制
        if (fabsf(motor_pid.setpoint) > FLT_EPSILON) // 使用优化的浮点数比较
        {
            // --- 加锁 ---
            if (osMutexAcquire(motor_mutexHandle, osWaitForever) == osOK)
            {
