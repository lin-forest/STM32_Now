...
INTEGRAL_LIMIT = 300               # 积分限幅，防 windup
INPUT_LIMIT = 1.0                  # 输入速度限幅 (m/s), 通过 speed 命令修改
MAX_MPS = 0.8                      # 驱动→电机映射系数（固定）
...
running = True
wheel_override = [False, False, False, False]   # 各单元是否手动覆盖
wheel_override_target = [0.0, 0.0, 0.0, 0.0]    # 手动覆盖目标角度
...
def serial_reader():
    """串口遥控读取线程"""
    global serial_remote_vx, serial_remote_vy, serial_remote_omega, serial_remote_updated, INPUT_LIMIT, target_steer_deg, target_drive_speed, angle_mode, wheel_override, wheel_override_target
    ...
    while running:
        ...
        try:
            ...
            # ── 命令处理 ──
            # ── 机械臂控制 ──
            if parts[0] == 'arm' and len(parts) > 1:
                try:
                    # 首次使用时导入并初始化
                    if not hasattr(serial_reader, '_arm'):
                        # 尝试多个路径（开发机 / Mini PC）
                        for p in ['/home/lin/Lin_workspace/Lin_rs00',
                                  '/home/lin/rs00',
                                  os.path.expanduser('~/rs00')]:
                            if os.path.isdir(p):
                                sys.path.insert(0, p)
                                break
                        from rs00_arm import RS00Arm
                        serial_reader._arm = RS00Arm()
                        serial_reader._arm.enable()
                        serial_reader._arm.enable_csp(speed_limit=0.5)
                        # 预设位置（角度制）
                        serial_reader._arm_presets = {
                            1: (-3, 6),      # 安全归零
                            2: (-30, 20),    # 轻度伸展
                            3: (-45, 30),    # 常规操作
                            4: (-60, 50),    # 中等伸展
                    ...
                    if sub == 'pos':
                        s = arm.get_state()
                        if s['shoulder']:
                            msg = (f"[POS] {s['shoulder']['position']:.0f} "
                                   f"{s['elbow']['position']:.0f}\n")
                            ser.write(msg.encode())
                        continue

                    # arm pump on/off — 气泵控制
                    if sub == 'pump' and len(parts) > 2:
                        if parts[2] == 'on':
                            can_send(0x141, bytes([0x64, 0x64, 0, 0]))
                            print(f"[遥控] 🌀 气泵开启")
                        elif parts[2] == 'off':
                            can_send(0x141, bytes([0x00, 0x00, 0, 0]))
                            print(f"[遥控] 🌀 气泵关闭")
                        continue
                    ...
                    if sub == 'temp':
                        s = arm.get_state()
                        if s['shoulder'] and s['elbow']:
                            msg = (f"[TEMP] 肩{s['shoulder']['temperature']:.0f}C "
                                   f"肘{s['elbow']['temperature']:.0f}C "
                                   f"力矩{s['shoulder']['torque']:.2f}/{s['elbow']['torque']:.2f}Nm\n")
                            ser.write(msg.encode())
                            print(f"[遥控] 🌡️ {msg.strip()}")
                        continue

                    # arm init — 初始化机械臂（使能+切CSP）
                    if sub == 'init':
                        if not hasattr(serial_reader, '_arm'):
                            continue  # 上面的初始化循环会处理
                        print(f"[遥控] 🦾 机械臂已就绪，按 arm home 归零")
                        continue

                    # arm speed <0.01~0.1> — 调速
                    if sub == 'speed' and len(parts) > 2:
                        spd = max(0.01, min(0.1, float(parts[2])))
                        from rs00_control import write_param
                        import struct
                        write_param('can0', 0x7017, struct.pack('<f', spd), 1)
                        write_param('can0', 0x7017, struct.pack('<f', spd), 2)
                        print(f"[遥控] 🦾 臂速 = {spd:.2f} rad/s")
                        continue

                    # arm preset save <N> — 保存预设
                    if sub == 'preset' and len(parts) > 2:
                        if parts[2] == 'save' and len(parts) > 3:
                            n = int(parts[3])
                            s = arm.get_state()
                            serial_reader._arm_presets[n] = (
                                s['shoulder']['position'],
                                s['elbow']['position'] if s['elbow'] else 0
                            )
                            print(f"[遥控] 🦾 预设 #{n} 已保存")
                        else:
                            n = int(parts[2])
                            if n in serial_reader._arm_presets:
                                pos = serial_reader._arm_presets[n]
                                r = arm.set_angles_csp(pos[0], pos[1]).verify()
                                print(f"[遥控] 🦾 预设 #{n} → {pos[0]:.0f}° {pos[1]:.0f}°")
                                if not r.get('success', False):
                                    arm.disable()
                                    print(f"[遥控] ⛔ 堵转！臂已停止")
                            else:
                                print(f"[遥控] ⚠️ 预设 #{n} 不存在")
                        continue

                    # arm free — 完全放松（disable，零力矩）
                    if sub == 'free':
                        from rs00_control import motor_disable
                        motor_disable('can0', 1, 0xFD)
                        motor_disable('can0', 2, 0xFD)
                        arm._enabled = False
                        arm._csp_mode = False
                        print(f"[遥控] 🦾 已 disable（零力矩，机械臂可自由掰动）")
                        print(f"[遥控]   重新控制需发: arm lock")
                        continue

                    # arm lock — 重新使能+切CSP
                    if sub == 'lock':
                        arm.enable()
                        arm.enable_csp(speed_limit=0.5)
                        s = arm.get_state()
                        sp = s['shoulder']['position'] if s['shoulder'] else -3
                        ep = s['elbow']['position'] if s['elbow'] else 6
                        ep = max(ep, 8)  # 肘部最低 8°，避免收缩堵转
                        arm.set_angles_csp(sp, ep)
                        print(f"[遥控] 🦾 CSP锁死 @ {sp:.0f}° {ep:.0f}°")
                        continue
                    ...
                    if sub == 'home':
                        r = arm.home().verify()
                        if not r.get('success', False):
                            print("[遥控] ⚠️ 归零失败，可能堵转！disable...")
                            arm.disable()
                            print("[遥控] ⛔ 臂已停止")
                        continue

                    # arm s <°> — 单关节：肩膀
                    if sub == 's' and len(parts) > 2:
                        s_deg = float(parts[2])
                        r = arm.set_angles_csp(s_deg, None).verify()
                        print(f"[遥控] 🦾 肩 {s_deg:.0f}°")
                        if not r.get('success', False):
                            arm.disable()
                            print(f"[遥控] ⛔ 堵转！臂已停止")
                        continue
                    ...
                    # arm e <°> — 单关节：肘部
                    if sub == 'e' and len(parts) > 2:
                        e_deg = float(parts[2])
                        r = arm.set_angles_csp(None, e_deg).verify()
                        print(f"[遥控] 🦾 肘 {e_deg:.0f}°")
                        if not r.get('success', False):
                            arm.disable()
                            print(f"[遥控] ⛔ 堵转！臂已停止")
                        continue
                    ...
                    # arm <肩°> <肘°> — 绝对位置
                    # arm +<肩°> <肘°> — 增量
                    p1, p2 = parts[1], parts[2] if len(parts) > 2 else '0'
                    if p1.startswith('+'):
                        # 增量模式（以 + 开头才是增量）
                        s = arm.get_state()
                        cur_s = s['shoulder']['position']
                        cur_e = s['elbow']['position'] if s['elbow'] else 0
                        delta_s = float(p1)
                        delta_e = float(p2)
                        r = arm.set_angles_csp(cur_s + delta_s, cur_e + delta_e).verify()
                        print(f"[遥控] 🦾 增量 {delta_s:+.0f}° {delta_e:+.0f}° → "
                              f"{cur_s+delta_s:.0f}° {cur_e+delta_e:.0f}°")
                        if not r.get('success', False):
                            arm.disable()
                            print(f"[遥控] ⛔ 堵转！臂已停止")
                    else:
                        ...
                        s_deg = float(p1)
                        e_deg = float(p2)
                        r = arm.set_angles_csp(s_deg, e_deg).verify()
                        print(f"[遥控] 🦾 {s_deg:.0f}° {e_deg:.0f}°")
                        if not r.get('success', False):
                            arm.disable()
                            print(f"[遥控] ⛔ 堵转！臂已停止")
                except Exception as e:
            ...
            if parts[0] == 'speed' and len(parts) > 1:
                try:
                    ...
                    INPUT_LIMIT = max(0.1, min(2.0, new_limit))
                    print(f"[遥控] 🏎️ 速度限幅 = {INPUT_LIMIT:.1f} m/s   "
                          f"(W=全速, 1/3限幅={INPUT_LIMIT/3:.2f} m/s)")
                except: pass
                continue
            if parts[0] == 'wheel' and len(parts) > 2:
                try:
                    u = int(parts[1]) - 1
                    if 0 <= u < 4:
                        if parts[2] in ('hold', 'h'):
                            wheel_override[u] = True
                            wheel_override_target[u] = angle_now(u)
                            print(f"[遥控] 🛞 UNIT{u+1} 保持当前角度 {wheel_override_target[u]:.0f}°")
                        elif parts[2] in ('free', 'f'):
                            wheel_override[u] = False
                            print(f"[遥控] 🛞 UNIT{u+1} 释放控制")
                        elif parts[2].startswith('+') or parts[2].startswith('-'):
                            delta = float(parts[2])
                            wheel_override[u] = True
                            wheel_override_target[u] = (wheel_override_target[u] + delta) % 360
                            print(f"[遥控] 🛞 UNIT{u+1} 增量 {delta:+.0f}° → {wheel_override_target[u]:.0f}°")
                        else:
                            angle = float(parts[2])
                            wheel_override[u] = True
                            wheel_override_target[u] = angle % 360
                            print(f"[遥控] 🛞 UNIT{u+1} 设角度 {wheel_override_target[u]:.0f}°")
                        # 不设驱动速度，让转向先到位
                except: pass
                continue
...
def apply_remote_command():
    """将串口遥控的最新值应用到目标"""
    global target_steer_deg, target_drive_speed, angle_mode, serial_remote_vx, serial_remote_vy, serial_remote_omega, serial_remote_updated, last_remote_time, INPUT_LIMIT, MAX_MPS, wheel_override
    ...
    # 安全检查：限制最大速度（受 speed 命令控制）
    vx = max(-INPUT_LIMIT, min(INPUT_LIMIT, vx))
    vy = max(-INPUT_LIMIT, min(INPUT_LIMIT, vy))
    omega = max(-INPUT_LIMIT, min(INPUT_LIMIT, omega))

    results = inv_kinematics(vx, vy, omega)
    for u, (steer, drive) in enumerate(results):
        if not wheel_override[u]:
...
def print_status():
    global drive_ready, wheel_override, wheel_override_target
    print("-" * 85)
    ...
    for u in range(4):
        ...
        if alive:
            ...
            ovr = "🛞" if wheel_override[u] else "  "
            tgt = wheel_override_target[u] if wheel_override[u] else target_steer_deg[u]
            print(f"  UNIT{u+1}  {angle:7.1f}°  {tgt:4.0f}°  "
                  f"{target_drive_speed[u]:+4d}  "
                  f"T=0x{tf:02X} P=0x{pf:02X}  {prot}  {flag}{ovr}")
        else:
            print(f"  UNIT{u+1}        离线")
...
def main():
    global can_bus, running, drive_ready, DISABLED_UNITS, last_remote_time, angle_mode, target_drive_speed, REMOTE_MODE, wheel_override, wheel_override_target, MAX_MPS
    global SERIAL_PORT, SERIAL_BAUD, CAN_BITRATE, SERIAL_ENABLED
        ...
        while running:
            ...
            for u in range(4):
                ...
                if PROTECTION_ACTIVE[u]:
                    continue
                angle = angle_now(u)
                target = wheel_override_target[u] if wheel_override[u] else target_steer_deg[u]
                err = shortest_error(angle, target)
            ...
            if angle_mode:
                for u in range(4):
                    if u in DISABLED_UNITS or PROTECTION_ACTIVE[u]:
                        continue
                    target_now = wheel_override_target[u] if wheel_override[u] else target_steer_deg[u]
                    err = shortest_error(angle_now(u), target_now)
