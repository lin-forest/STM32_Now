...
### 通信
- [x] CAN1：500kbps, RX0+SCE 中断, 过滤全收
- [x] CAN 0x130：ARM_CMD 角度控制（int16 LE, 0.1°精度, -150°~+150°）
- [x] CAN 0x430：ARM_CONFIG 回中(0x01) / 设速度(0x02)
- [x] printf 调试输出
...
| CAN_Rx_Ta | `CAN_Rx_Task_Run` | Normal (24) | 256 | 事件驱动 | ✅ 解析 0x130/0x430 |
| **Servo_Ta** | **`Servo_Task_Run`** | **Normal (24)** | **128** | **20ms** | **✅ 平滑+输出** |
| DC_Motor_Ta | `DC_Motor_Task_Run` | Normal1 (25) | 256 | — | ⏳ 空等 |
| Arm_State_Ta | `Arm_State_Task_Run` | Normal (24) | 256 | — | ⏳ 空等 |
...
typedef struct {
    ...
    int16_t j0_target;          // 预留
    int16_t j0_current_speed;   // 预留
} ArmState_t;
...
```
Byte 0:     0x11 = SET_POSITION
Byte 1:     关节ID (1=J1, 2=J2, 3=Gripper)
Byte 2..3:  目标值 (int16 LE, 0.1°精度)
            J1/J2: ±1500 对应 ±150.0°
            Gripper: 1000~5000 脉宽值
Byte 4..7:  保留 (填0)
```
...
| ID | 方向 | 功能 |
|:--:|:----:|------|
| 0x130 | Chassis→Arm | 关节角度控制 |
| 0x430 | Chassis→Arm | 参数配置 |
| 0x230 | Chassis→Arm | 状态查询 ⏳ |
| 0x330 | Arm→Chassis | 状态上报 ⏳ |
...
### P0 — Phase 5：CAN 状态上报
- [ ] **Arm_State_Task_Run** 实现 50ms 周期 CAN 上报 0x330
- [ ] 上报格式：J1角度(2B) + J2角度(2B) + Gripper(1B) + Flags(1B)
- [ ] **CAN_Rx_Task** 增加 0x230 查询处理
